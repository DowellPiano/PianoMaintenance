...
class DemoAutoLoginTests(CompanyScopedTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_demo_auto_login_allows_dashboard_without_manual_sign_in(self):
        ...
        self.assertEqual(self.client.session['active_company_id'], self.company.pk)
...
class DemoDataCommandTests(TestCase):
    def test_reset_demo_data_replaces_existing_records_with_company_scoped_demo(self):
        old_company = Company.objects.create(name='Old Company', slug='old-company')
        old_user = Technician.objects.create_user(
            username='old-user',
            password='StrongPass123',
        )
        CompanyMembership.objects.create(company=old_company, user=old_user)

        out = StringIO()
        call_command('reset_demo_data', '--yes', stdout=out)

        self.assertIn('Demo data reset complete.', out.getvalue())
        self.assertFalse(Company.objects.filter(slug='old-company').exists())
        company = Company.objects.get(slug='overtone-demo')
        demo_admin = Technician.objects.get(username='demo-admin')
        membership = CompanyMembership.objects.get(company=company, user=demo_admin)
        self.assertTrue(membership.role_admin)
        self.assertTrue(membership.role_technician)
        self.assertEqual(
            CompanySettings.objects.get(company=company).company_name,
            'Overtone Piano Service',
        )
        self.assertGreaterEqual(Piano.objects.filter(company=company).count(), 6)
        self.assertGreaterEqual(WorkOrder.objects.filter(company=company).count(), 5)
        self.assertGreaterEqual(MaintenanceRequest.objects.filter(company=company).count(), 2)
        self.assertGreaterEqual(Part.objects.filter(company=company).count(), 4)
