...
from django.db.models import Count, DecimalField, F, Max, Sum, Q
...
# Calendar event display constants
STATUS_COLOR = {
    'Open': '#3b82f6',
    'In Progress': '#f59e0b',
    'Complete': '#22c55e',
...
class ScheduleTemplateViewSet(viewsets.ModelViewSet):
    ...
    def apply_to_pianos(self, request, pk=None):
        ...
        created_ids = []
        for piano in Piano.objects.filter(id__in=piano_ids):
        ...
        return Response({'created': len(created_ids), 'schedule_ids': created_ids})
...
class WorkOrderViewSet(viewsets.ModelViewSet):
    ...
    def complete(self, request, pk=None):
        ...
        if work_order.status in (WorkOrder.Status.COMPLETE, WorkOrder.Status.CANCELLED):
            return Response(
        ...
        technician = request.user if request.user.is_authenticated else work_order.assigned_tech
        if not technician:
            return Response(
...
class PartUsedViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        cost_at_time = serializer.validated_data.get('cost_at_time') or part.unit_cost
...
class AttachmentViewSet(PhotoViewSet):
...
class ReportsViewSet(viewsets.ViewSet):
    ...
    def _build_filter_q(self, filter_dict):
        q = Q()
        for key, val in filter_dict.items():
            q &= Q(**{key: val})
        return q

    def _get_technician_stats(self, request):
        log_filters = self._build_log_filter(request)
        return (
            Technician.objects
            .annotate(
                total_hours=Coalesce(
                    Sum('logs__hours_worked', filter=self._build_filter_q(log_filters)),
                    Decimal('0'),
                ...
                work_order_count=Count(
                    'logs__work_order',
                    distinct=True,
                    filter=self._build_filter_q(log_filters),
                ),
                last_logged_at=Max(
                    'logs__logged_at',
                    filter=self._build_filter_q(log_filters),
    ...
    def technicians(self, request):
        techs = self._get_technician_stats(request)
        data = [
            {
                'id': t.pk,
...
def calendar_events(request):
    ...
    for wo in work_orders:
        is_overdue = (
            ...
            wo.status not in (WorkOrder.Status.COMPLETE, WorkOrder.Status.CANCELLED)
        )
        events.append({
            'id': f'wo_{wo.id}',
            'type': 'work_order',
...
def piano_profile(request, piano_id):
    ...
    return Response({
        'piano': PianoSerializer(piano, context=ctx).data,
        ...
        'photos': PhotoSerializer(photos, many=True, context=ctx).data,
    })
